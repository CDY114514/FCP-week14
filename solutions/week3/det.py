#!/usr/bin/env python3
# Calculate the determinant of a 3x3 matrix: https://youtu.be/0c7dt2SQfLw?t=132
...
def determinant(M):
    ...
    if N == 1:
        N2 = len(M[0])
        # Handle a matrix with no elements: [[]]
        if N2 == 0:
            return 1
        # Handle a 1x1 matrix: [[1]]    
        elif N2 == 1:
            return M[0][0]
    elif N == 2:
...
def test_determinant():
    ...
    # Adding new tests for determinants:
    assert determinant([[2]]) == 2
    assert determinant([[5]]) == 5
    assert determinant([[]]) == 1

def test_minor():
    # Adding test for minor:
